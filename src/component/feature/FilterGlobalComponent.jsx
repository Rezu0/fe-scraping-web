...
function FilterGlobalComponent() {
  ...
  const clickFilterGenreBox = () => {
    setIsFilterGenre((prev) => !prev);
  }
  return (
    <>
      <div className="grid grid-cols-1 gap-4 mt-5">
        <div className="p-4 text-end">
          ...
          <div className={classNames(
            isFilterGenre ? "translate-x-0 opacity-100 pointer-events-auto" : "translate-x-full opacity-0 pointer-events-none",
            "fixed top-1/3 right-5 sm:right-10 md:right-20 -translate-y-16 bg-black border border-adultdesu-navbartext py-2 px-4 mt-1 rounded-md w-5/6 max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg text-sm text-start transition-all duration-300"
          )}>
