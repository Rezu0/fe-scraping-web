...
function ButtonServerComponent({ isServerVideo, isServerAcitve, setServerActive }) {
  const [isServer, setServer] = useState([]);
  const [isActive, setActive] = useState(null);

  useEffect(() => {
    setServer(isServerVideo);
    setActive(isServerAcitve);
  }, [isServerAcitve, isServerVideo])

  const handlerClickButtonServer = (link) => {
    setServerActive(link);
  }
    ...
    <>
      {isServer?.filter(item => item !== null).map((item, index) => (
        <div 
          ...
          onClick={() => handlerClickButtonServer(item)}
        >
